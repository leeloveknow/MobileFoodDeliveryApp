...
class OrderPlacement:
    ...
    def save_order_history(self, order_id, items, total_amount, status):
        """
        Saves the order history with the given details.

        Args:
            order_id (str): Unique identifier for the order.
            items (list): List of items in the order.
            total_amount (float): Total amount of the order.
            status (str): Status of the order (e.g., "delivered", "pending").
        """
        order_history = {
            "order_id": order_id,
            "items": items,
        ...
        self.user_profile.order_history.append(order_history)

    def get_order_history(self, filter_by=None, filter_value=None):
        """
        Retrieves the order history based on the given filter.

        Args:
            filter_by (str): The attribute to filter by (e.g., "date", "status").
            filter_value (str): The value to filter by.

        Returns:
            list: A list of orders that match the filter criteria.
        """
        if filter_by and filter_value:
            return [order for order in self.user_profile.order_history
                    if order[filter_by] == filter_value]
        return self.user_profile.order_history
...
class TestCart(unittest.TestCase):
    ...
    def test_add_item_update_quantity(self):
        # 添加一个新商品到购物车
        self.cart.add_item("Apple", 0.99, 2)
        # 再次添加相同的商品，数量为3
        self.cart.add_item("Apple", 0.99, 3)
        # 检查商品数量是否更新为5
        apple_item = next((item for item in self.cart.items if item.name == "Apple"), None)
